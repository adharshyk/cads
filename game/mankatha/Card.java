...
import day6.cards.InvalidRankOrSuitException;
	...
	public Card(int rank, int suit) {
		try {
			if ((rank >= ACE && rank <= KING) && (suit >= SPADES && suit <= CLUBS)) {
				...
				throw new InvalidRankOrSuitException("Invalid rank and/or suit; Card initialized with zero values");
			}
		} catch (InvalidRankOrSuitException e) {
	...
	public void setRank(int rank) {
		try {
			if (rank >= ACE && rank <= KING) {
				...
				throw new InvalidRankOrSuitException("Invalid rank value");
			}
		} catch (InvalidRankOrSuitException e) {
			System.out.println(e.getMessage());
		}
	...
	public void setSuit(int suit) {
		try {
			if (suit >= SPADES && suit <= CLUBS) {
				...
				throw new InvalidRankOrSuitException("Invalid suit value");
			}
		} catch (InvalidRankOrSuitException e) {
			System.out.println(e.getMessage());
		}
